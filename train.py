...
def init_from_scratch(args, train_exs, dev_exs):
    ...
    word_dict, char_dict = utils.build_word_char_dict(args, train_exs) # Whether add dev_exs to build word dict.
    logger.info('Num words = %d' % len(word_dict))
    logger.info('Num characters = %d' % len(char_dict))

    #Initialize model
    model = DocReader(args, word_dict, char_dict, feature_dict)
